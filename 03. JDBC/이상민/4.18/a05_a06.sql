...
SELECT * FROM vwstucou where vcouseq =1;
                                    ...
                                    INNER JOIN tbltextbook t
                                        ON t.seq = s.textbookseq
                                            INNER JOIN tblsubjectname j
                                                ON j.seq = s.subjectnameseq
                                                    order by s.seq;
 select * from tblsubject where courseseq=2
 ;                                          
--얻어온 과정seq, 과목 seq 이용
...
select 
    case
        when q.count(*)>(select count(*) from vwallcount where vsubseq=1) then 'o'
        when q.count(*)<(select count(*) from vwallcount where vsubseq=1) then 'x'
    end as vcnt 
     from tblQuiz q where subjectseq = 1;



create view vwallcount
as
    ...
    INNER JOIN tblsubjectgrade g
        ON s.seq = g.studentseq
            INNER JOIN tblstudentinfo i
                ON i.seq = s.studentinfoseq
                    where i.state=1;
                        WHERE s.courseseq=1 AND g.subjectseq = 1 and s.state=1;


--A06005
--학생 검색
SELECT * FROM tblstudentinfo WHERE name like '%'||'문윤하'||'%'; 

SELECT * FROM vwcousubstutexlec
WHERE vcouseq = 1 and vsinfoseq =1;

select * from vwstucoutemp where vsinfoseq = 1;
            ...
            INNER JOIN tblclassroom r  --강의실
                 ON c.classroomseq=r.seq
                    INNER JOIN tbllecturer l  --강사
                       ...
                       SELECT * FROM vwsinfogra WHERE  vsubseq = 7; 
select * from tblsubjectgrade;
select * from tblQuiz;
select * from tblsubject;
select count(*) as vcnt from tblQuiz where subjectseq = 7;
select * from tblsubjectgrade where subjectseq = 7;

select * from vwsubgraquiz where vsubseq=3 and vsinfoseq = 3;
CREATE or replace VIEW vwsubgraquiz
as
SELECT distinct  r.name as vroomname, e.name as vsubname, to_char(j.startdate,'yyyy-mm-dd') as vsubstart, to_char(j.enddate,'yyyy-mm-dd') as vsubend,
        t.name as vtextname, l.name as vlecname,i.seq as vsinfoseq,j.seq as vsubseq,q.quizdate as vquizdate, 
        g.written as vgrawritten, g.practical as vgraprac,j.practicalpercent as vsubpracper,j.writtenpercent as vsubwripercent
         FROM tblcourse c   --과정
           INNER JOIN tblcoursename n  --과정명
             ON c.coursenameseq = n.seq
            INNER JOIN tblclassroom r  --강의실
...
--CREATE or replace VIEW vwsubgraquiz
--as
--SELECT      j.name as vsubname,to_char(j.startdate,'yyyy-mm-dd') as vsubstart,to_char(j.enddate,'yyyy-mm-dd'),

select * from vwsubgraquiz  WHERE vsubseq= 3 and vstuinseq = 1;

SELECT * FROM vwcousubstutexlec WHERE vsinfoseq =1;
                                                        ...
                                                        inner join tbltextbook t
                                                          on  t.seq = j.textbookseq
                                                            Inner join tblsubjectname n 
                                                                ...
                                                                where i.state=1;
                                                                     WHERE i.seq = 3 and j.seq = 3;
                                                         commit;            
                                  select*from tblstudentinfo;       
                                  SELECT * FROM tblstudentinfo WHERE name like '%'||'왕'||'%';
----------------------------------------
select * from tblstudent where studentinfoseq =1 ;
select * from vwbeforecou;
insert into tblstudent values(studentseq.nextval,1,1,sysdate,default,null);

delete tblstudent where courseseq=1 and seq =72;
select * from tblstudentinfo;

select * from vwdellist where vstuinfoseq=61;
select * from vwdellist where vstuinfoseq=1;
select * from vwdellist;
select * from tab;
...
SELECT * FROM vwtblstudentinfo WHERE name like '%'||'왕'||'%';
UPDATE tblstudent SET status = 2, statusdate = sysdate WHERE seq = 1; 
select * from tblstudent;